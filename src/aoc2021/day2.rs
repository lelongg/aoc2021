...
impl Position {
    fn apply_command(mut self, command: Command) -> Self {
        ...
        match command.direction {
            Forward => self.horizontal_position += command.magnitude,
            Up => self.depth -= command.magnitude,
            Down => self.depth += command.magnitude,
        }
        self
    }
}
...
#[aoc(day2, part2)]
fn part2(_input_data: &str) -> Option<u32> {
    None
}
...
fn example() {
    ...
    down 8
    forward 2";
    assert_eq!(part1(&input_data), Some(150));
    assert_eq!(part2(&input_data), None);
}
